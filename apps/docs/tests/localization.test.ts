...
import {normalizeAdapterCopy} from "@generative-a11y/core/messages";
...
it("runs the complete illustrative catalog with independently tagged answer text", () => {
  const recorder = createRecorder({messages:frenchCatalog,policy:{minimumGapMs:0,text:{minimumCharacters:1}}});
  ...
  expect(normalizeAdapterCopy(frenchAdapterCopy)).toEqual(frenchAdapterCopy);

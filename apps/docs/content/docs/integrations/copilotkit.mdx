...
export const searchableText = [
  ...
  "tsx",
  "const bindingScopeId = useId(); const { agent, isReady } = useAgent({ agentId }); useEffect(() => { if (!isReady) return; const binding = bindAgent({ agent, runtime, scopeId: `copilotkit:${bindingScopeId}`, }); return () => binding.dispose(); }, [agent, isReady, runtime, bindingScopeId]);",

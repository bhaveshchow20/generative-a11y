...
section: "Concepts"
keywords: ["localization", "i18n", "language", "catalog", "announcementCatalog", "AdapterAnnouncementCopy"]
related: ["/api/core/create-generative-a11y", "/api/core/events", "/docs/testing/replay"]
order: 8
---

export const sourceMetadata = { publicPath: "/docs/localized-announcements", section: "Concepts", related: ["/api/core/create-generative-a11y", "/api/core/events", "/docs/testing/replay"], order: 8 };
export const searchableText = "Localized announcements. announcementCatalog englishAnnouncementCatalog AnnouncementCatalog AnnouncementMessages AnnouncementMessageParameters AnnouncementMessageId AdapterAnnouncementCopy normalizeAdapterAnnouncementCopy. Reuse host i18n translations and pluralization. Separate response locale from notice language. Complete typed catalog, synchronous formatters, safe English fallback, construction ownership, replay requires matching configuration. No translation engine or language detection.";
...
[Try “Stream with French notices” in the lifecycle lab](/examples/lifecycle-lab). The visible answer remains English. Its response-text intents use `en`, while completion uses the French catalog's `fr`. The example is illustrative copy, not a reviewed translation pack or proof of screen-reader pronunciation.
...
const runtime = createGenerativeA11y({ announcementCatalog: catalog });
...
React's `GenerativeA11yProvider` accepts `announcementCatalog` through its existing options. For a supplied `runtime`, configure the catalog on that runtime; the provider borrows it and does not reconfigure it. Mount the provider with a stable catalog. Change language at an intentional runtime/provider replacement boundary; do not replace an active runtime on every render.

## Existing framework adapters

Pass `copy` to `createObserver`, `bindThreadRuntime`, `bindAgent`, or `useChatAccessibility`. This optional `AdapterAnnouncementCopy` contains `locale`, `toolLabel`, `approvalRequested`, `approvalResolved` (approved/rejected/cancelled), `inputRequested`, and `inputResolved` (submitted/cancelled). Each adapter uses only the fields for events it already recognizes.
...
const runtime = createGenerativeA11y({ announcementCatalog: frenchCatalog });
...
The copy is validated, copied and frozen before subscribing. `normalizeAdapterAnnouncementCopy(copy)` is also exported from core for custom adapter authors; it returns that validated immutable copy and throws `TypeError` for invalid configuration. It performs no translation.

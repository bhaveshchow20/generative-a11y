...
section: "Concepts"
keywords: ["attention", "quiet", "override", "reading-history", "useAttentionControl", "bindAttention"]
...
The provider renders no visible controls. The example below adds host-owned buttons using `useAttentionControl()`, which returns `{ state: { observed, override, effective }, setOverride }`. Explicit `"quiet"` and `"normal"` override observed attention; `"auto"` restores policy-based behavior. With core attention disabled, the effective mode remains normal.
...
import { ManualClock, type AnnouncementIntent, type AttentionOverride, type RuntimeEvent } from "@generative-a11y/core";
import {
  A11yProvider,
  useAttentionControl,
  useAttentionRefs,
    ...
    <section className="lab" aria-labelledby="attention-lab-title">
      ...
      <div className="lab-grid">
        <section className="host-surface" aria-label="Attention example host interface">
          <div ref={bindings.conversationRef} tabIndex={0} aria-label="Conversation history" role="region">
            <p>Earlier message: prepare a report and ask before publishing.</p>
            <article ref={bindings.newestResponseRef} aria-label="Newest response"><p>{text || "The response will appear here."}</p></article>
          </div>
          <label htmlFor="attention-composer">Your message</label>
          <textarea id="attention-composer" ref={bindings.composerRef} />
...
const attentionScenario: readonly RuntimeEvent[] = [

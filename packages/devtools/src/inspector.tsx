...
function correlation(record: DevtoolsRecord) {
  ...
  if (record.responseId)
    return `${prefix}response:${record.responseId}:${record.responseInstanceId ?? "*"}`;
  if (record.toolId)
    return `${prefix}tool:${record.toolId}:${record.toolInstanceId ?? "*"}`;
...
function Detail({
  ...
  const source = snapshot.runtimeSources[record.runtimeId];
...
export function DevtoolsInspector({
  ...
  const visible = snapshot.records
    ...
    .reverse();
  const selected =
    snapshot.records.find((record) => key(record) === selectedKey) ??
    visible[0];
    ...
    >
      ...
      >
          ...
          <ScrollArea className="ga-trace-scroll">
            <List
              ...
              selected={key(
                selected ??
                  visible[0] ??
                  ({ captureSequence: -1 } as DevtoolsRecord),
              )}
            />
          </ScrollArea>
        </ResizablePanel>
        ...
        <ResizablePanel defaultSize={52} minSize={34}>
          <Detail record={selected} related={related} store={store} />
        </ResizablePanel>
      </ResizablePanelGroup>
    </section>
  );
